...
{
    [System.Serializable]
    public class ChangeSPAction : EventAction
    {
        [Tooltip("Specifies the Unit's SP to change.")]
        ...
        [Header("Operand")]
        [Tooltip("Constant SP value to change.")]
        public int constant = 0;
        public ChangeSPAction()
        {
            eventName = "Change SP";
        ...
        private void CalculateValue(PartyMember member)
        {
            if (operation == AddSetOperationType.Add)
                member.CalculateSP(constant);
            else if (operation == AddSetOperationType.Set)
                member.SetSP(constant);
        }
    }
}
